...
%--Get_ip: Ubuntu version--%

get_ip(Ip):- 
	process_create(path(hostname),['-I'],[stdout(pipe(O))]),
	read_stream_to_codes(O,C1),
	delete(C1,10,C2),delete(C2,32,C3),
	string_codes(S,C3),
	atom_string(Ip,S),close(O).
...
%% ------------------------------------------------------------------------------------------------------------
%% ---- Tartarus Peripheral Interface code starts here
%% ----- Embedded into single file for portability
%% ----- As of now raspberry Pi is supported. 
%% ----- WiringPi Prolog Library must be installed for this to work 




%% Start the peripheral interface
start_peripherals :- 
    writeln('Initializing Tartarus Peripheral Interface'),
    tpi_init.


%% Tartarus Peripheral interface
%% Author: Midhul Varma <midhul.v@gmail.com>
%% TODO: Take care of synchronization

:- use_module(library(persistency)).

:- persistent device(devid:atom, iface:atom, aux:any, api:atom).
...
tpi_init :- 
    ...
    directory_file_path(SwiHome, 'wipi/corefunc.so', Corefunc),
    load_foreign_library(Corefunc),
    directory_file_path(SwiHome, 'wipi/i2c.so', I2c),
    load_foreign_library(I2c),
    directory_file_path(SwiHome, 'wipi/setup.so', Setup),
    load_foreign_library(Setup),
    directory_file_path(SwiHome, 'wipi/specifics.so', Specifics),
    load_foreign_library(Specifics),
    directory_file_path(SwiHome, 'wipi/timing.so', Timing),
    load_foreign_library(Timing),
    directory_file_path(SwiHome, 'wipi/interrupt.so', Interrupt),
    load_foreign_library(Interrupt),
    directory_file_path(SwiHome, 'wipi/modes.pl', Modes),
    consult(Modes),
    wiringPiSetup,
    absolute_file_name('devices.db', File, [access(write)]),
    db_attach(File, []),
    %% Initialize all devices
    forall(device(DevId,DevClass,_,Api), (
            (Api=nothing -> true ; consult(Api),tpi_init_device(DevClass, DevId)),
            mutex_create(DevId)           
        )).

%% Register device
register_device(DevId, Iface, Aux) :- 
    assert_device(DevId, Iface, Aux, nothing),
    mutex_create(DevId).

register_device(DevId, Iface, Aux, Api) :- 
    consult(Api),
    absolute_file_name(Api, FApi),
    assert_device(DevId, Iface, Aux, FApi),
    mutex_create(DevId),
    tpi_init_device(Iface, DevId).

%% Get device
get_device(DevId, Iface, Aux) :- 
    device(DevId, Iface, Aux, _).

%% Remove device
remove_device(DevId, Iface, Aux) :- 
    mutex_destroy(DevId),
    retract_device(DevId, Iface, Aux, _).

remove_device_all(DevId, Iface, Aux) :-
    forall(get_device(DevId, Iface, Aux), (mutex_destroy(DevId))), 
    retractall_device(DevId, Iface, Aux, _).

%% Execute goal aotmically
device_exec_atomic(DevId, Goal) :- 
    with_mutex(DevId, Goal).
