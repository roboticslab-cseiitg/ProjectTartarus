...
agent_post(platform,(Ip_receiver,Port_receiver),FunctionList):-                         %% +Receiver_Agent +Ip_receiver +Port_receiver +Sender_agent +Function
        ...
        catch(send_message(Ip_receiver,Port_receiver,FunctionList,Ack),Error,(writeln(Error),mutex_unlock(send_message_mutex),fail)),   
        (Ack->                                                                                                    
                true;                                                                                 
...
create_mobile_agent(GUID,Handler,List):- check_syntax(create_mobile_agent,_GUID,Handler,List), check_handler(Handler,3),  %added by Menaxi J Bagchi% 
...
handler(platform,(Ip,Port),send_ackm(GUID,_X)):-
        ...
        agent_post(platform,(Dest_ip,Dest_port),[predicate_tar,_Receive,(localhost,50),GUID,Source_ip,Source_port,Dest_ip,Dest_port,Ya2,Start_time,Time_taken]),
        agent_post(platform,(Dest_ip,Dest_port),[predicate_tar1,_Receive1, (localhost,60), GUID,Time_taken]),
...
handler( platform, (Ip_sender,Port_sender), send(GUID,Handler,PayloadList,[Code|CodeList],ListOfTokens,_BackTime)):-                            %//
        ...
        %writeln(Platform_Ip), writeln(Platform_Port),
        assert(agent_payload(GUID,PayloadList)),
        assert(agent_token(GUID,ListOfTokens)),
...
predicate_tar(_Receive,(localhost,50), GUID,Source_ip,Source_port,Dest_ip,Dest_port,Ya2,Start_time,Time_taken):- 
         %writeln(Source_ip), writeln(Source_port), writeln(Dest_ip), writeln(Dest_port),writeln(Ya2), writeln(Start_time),writeln(Time_taken), writeln(GUID), 
