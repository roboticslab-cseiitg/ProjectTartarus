...
start_tartarus(Ip,Port,Token):-
        check_syntax(start_tartarus,_Ip,Port,Token),               %added by Menaxi J Bagchi      
        start_db,
...
get_tartarus_details(IP,Port,OS):-
        get_platform_details(IP,Port,OS).

get_tartarus_details(_IP,_Port,_OS):-
...
create_mobile_agent(GUID,Handler,List):- check_handler(Handler,3),                                                         %added by Menaxi J Bagchi% 
                                         check_syntax(create_mobile_agent,_GUID,Handler,List),agent_create(GUID,Handler),  %added by Menaxi J Bagchi% 
                                         add_token(GUID,List).
...
add_token(GUID,List):-
        ...
        assert(agent_token(GUID,NewList)),writeln('token added'),!.                    %% //
...
check_handler(H,3):-                                                                                      %added by Menaxi J Bagchi%
   (current_predicate(H/3),true),writeln('Handler is present'),!;writeln('Handler not present'),abort.		
%-----------------------------------------------------------------------------------------------------%
...
%======================================================================================================%
%-------------------------------------- For Windows ---------------------------------------------------%
tts(X):-cd('C:\\Program Files (x86)\\eSpeak\\command_line'),string_concat('espeak \"',X,Y),string_concat(Y,'\"',Z),shell(Z).  
%------------------------------------------------------------------------------------------------------%
%-------------------------------------- For Ubuntu ----------------------------------------------------%        
tts(X):-string_concat('espeak \'',X,Y),string_concat(Y,'\'',Z),shell(Z).
% -----------------------------------------------------------------------------------------------------%
